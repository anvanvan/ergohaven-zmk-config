...
- **Vial** is only reachable by reflashing **RMK** (`ergohaven/rmk-eh`) — that wipes the ZMK config, and the Vial GUI is USB-only anyway, so it buys nothing for wireless editing.
